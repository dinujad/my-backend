...
<div class="wrap">
  ...
  <div class="body">
    <div class="badge">Request #{{ $quoteRequest->request_number }}</div>
    <h2>Customer Information</h2>
    <table class="info-table">
      <tr><td>Name</td><td><strong>{{ $quoteRequest->customer_name }}</strong></td></tr>
      @if($quoteRequest->company_name)
      <tr><td>Company</td><td>{{ $quoteRequest->company_name }}</td></tr>
      @endif
      <tr><td>Email</td><td>{{ $quoteRequest->email }}</td></tr>
      <tr><td>Phone</td><td>{{ $quoteRequest->phone }}</td></tr>
      <tr><td>Contact Preference</td><td>{{ ucfirst($quoteRequest->preferred_contact) }}</td></tr>
      <tr><td>Urgency</td><td>{{ ucfirst($quoteRequest->urgency ?? 'normal') }}</td></tr>
      @if($quoteRequest->deadline)
      <tr><td>Deadline</td><td>{{ $quoteRequest->deadline->format('d M Y') }}</td></tr>
      @endif
    </table>

    <h2>Requested Items ({{ $quoteRequest->items->count() }})</h2>
      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            @if($item->item_notes)<br><small style="color:#666;">{{ $item->item_notes }}</small>@endif
          </td>
    ...
    <div style="margin-top:20px;padding:14px;background:#f9f9f9;border-radius:6px;font-size:14px;">
      <strong>Customer Notes:</strong><br>{{ $quoteRequest->customer_notes }}
    </div>
    @endif

    <a href="{{ config('app.frontend_url') }}/admin/quotes/{{ $quoteRequest->id }}" class="btn">
