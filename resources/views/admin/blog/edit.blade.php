        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                ...
                <div>
                    ...
                    <input name="slug" value="{{ old('slug', $blog->slug) }}"
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red">{{ old('excerpt', $blog->excerpt) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Content</label>
                    <textarea name="content" rows="14" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red font-mono">{{ old('content', $blog->content) }}</textarea>
                </div>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input name="seo_title" value="{{ old('seo_title', $blog->seo_title) }}" maxlength="60"
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">SEO Description</label>
                        <textarea name="seo_description" rows="2" maxlength="160" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red">{{ old('seo_description', $blog->seo_description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" {{ $blog->is_published ? 'checked' : '' }} class="rounded border-gray-300 text-brand-red">
                    <span class="text-sm text-gray-700">Published</span>
                </label>
                @if($blog->published_at)
                    <p class="text-xs text-gray-400">Published {{ $blog->published_at->format('d M Y') }}</p>
                @endif
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
                ...
                <div>
                    ...
                    <input name="featured_image" value="{{ old('featured_image', $blog->featured_image) }}"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-red">
                </div>
            </div>
