    ...
    <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-5 py-4 bg-gray-50 border-b border-gray-200">
        <div class="flex items-center gap-3">
          <span class="font-bold text-gray-900 text-base">{{ $zone->name }}</span>
          @if($zone->description)
            <span class="text-xs text-gray-500">— {{ $zone->description }}</span>
          @endif
          @if($zone->is_active)
        ...
        <div class="flex items-center gap-2">
          ...
          <form action="{{ route('admin.shipping.zones.destroy', $zone) }}" method="POST"
                onsubmit="return confirm('Delete zone {{ $zone->name }}? Methods inside will also be deleted.')">
            @csrf @method('DELETE')
            <button type="submit"
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="hover:bg-gray-50/60 transition">
            ...
            <td class="px-5 py-3">
              ...
              @else
                <span class="font-bold text-gray-800">Rs. {{ number_format($method->base_price, 2) }}</span>
                @if($method->free_shipping_threshold)
                  <p class="text-xs text-gray-400">Free if order ≥ Rs. {{ number_format($method->free_shipping_threshold, 2) }}</p>
                @endif
              @endif
            </td>
            <td class="px-5 py-3 text-gray-600">{{ $method->estimated_days ?? '—' }}</td>
            <td class="px-5 py-3">
              @if($method->is_active)
                <span class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-semibold text-green-700">Active</span>
              @else
            ...
            <td class="px-5 py-3 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <form action="{{ route('admin.shipping.methods.destroy', $method) }}" method="POST"
                      onsubmit="return confirm('Delete method {{ $method->name }}?')">
                  @csrf @method('DELETE')
                  <button type="submit"
                          class="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 transition">
                    <i class="bi bi-trash"></i>
      ...
      <div class="px-5 py-6 text-center text-sm text-gray-400">
        ...
        <a href="{{ route('admin.shipping.methods.create') }}?zone={{ $zone->id }}" class="text-brand-red font-semibold hover:underline ml-1">Add one →</a>
      </div>
      @endif
    </div>
    @empty
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="hover:bg-gray-50/60">
            <td class="px-5 py-2.5 font-semibold text-gray-900">{{ $district->name }}</td>
            <td class="px-5 py-2.5 text-gray-500">{{ $district->province }}</td>
