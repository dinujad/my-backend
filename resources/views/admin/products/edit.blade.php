...
<div x-data="productForm()" class="pb-20">
    <div class="flex items-center justify-between gap-3 mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ url('/product/' . $product->slug) }}" target="_blank" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition flex items-center gap-2">
    ...
    {{-- Standalone delete form (must not be nested inside the update form). Button uses form="..." to submit this. --}}
    <form id="admin-delete-product-form" method="POST" action="{{ route('admin.products.destroy', $product) }}" class="hidden" aria-hidden="true">
        @csrf
        @method('DELETE')
    </form>

    <form x-ref="mainForm" action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="flex flex-col lg:flex-row gap-6">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="8"
                              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-brand-red outline-none">{{ old('description', $product->description) }}</textarea>
                </div>
            </div>
                        ...
                        <div x-show="tab === 'inventory'" style="display: none;" x-transition.opacity>
                            <div class="space-y-4 max-w-lg">
                                <div class="flex items-center gap-4">
                                    ...
                                    <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 font-mono text-sm focus:border-brand-red outline-none">
                                </div>
                                <div class="flex items-center gap-4">
                                ...
                                <div class="flex items-center gap-4" x-show="manageStock">
                                    ...
                                    <input type="number" name="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 focus:border-brand-red outline-none" min="0">
                                </div>
                                <div class="flex items-center gap-4" x-show="manageStock">
                                    <label class="w-1/3 text-sm text-gray-600 font-medium">Low stock threshold</label>
                                    <input type="number" name="low_stock_threshold" value="{{ old('low_stock_threshold', $product->low_stock_threshold ?? 2) }}" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 focus:border-brand-red outline-none" min="0">
                                </div>
                                <div class="flex items-center gap-4" x-show="!manageStock">
                        ...
                        <div x-show="tab === 'shipping'" style="display: none;" x-transition.opacity>
                            <div class="space-y-4 max-w-lg">
                                <div class="flex items-center gap-4">
                                    ...
                                    <input type="number" step="0.001" name="weight" value="{{ old('weight', $product->weight) }}" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 focus:border-brand-red outline-none">
                                </div>
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="w-2/3 flex gap-2">
                                        <input type="number" step="0.1" name="length" value="{{ old('length', $product->length) }}" placeholder="Length" class="w-1/3 border border-gray-200 rounded-md px-2 py-1.5 text-sm focus:border-brand-red outline-none">
                                        <input type="number" step="0.1" name="width" value="{{ old('width', $product->width) }}" placeholder="Width" class="w-1/3 border border-gray-200 rounded-md px-2 py-1.5 text-sm focus:border-brand-red outline-none">
                                        <input type="number" step="0.1" name="height" value="{{ old('height', $product->height) }}" placeholder="Height" class="w-1/3 border border-gray-200 rounded-md px-2 py-1.5 text-sm focus:border-brand-red outline-none">
                                    </div>
                                </div>
                                <div class="flex items-center gap-4">
                                    ...
                                    <select name="shipping_class" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 focus:border-brand-red outline-none">
                                        ...
                                        <option value="lightweight" {{ old('shipping_class', $product->shipping_class) === 'lightweight' ? 'selected' : '' }}>Lightweight</option>
                                    </select>
                                </div>
                                <div class="flex items-center gap-4 pt-4">
                                    ...
                                    <label class="w-2/3 flex items-center gap-2 cursor-pointer text-gray-700 text-sm">
                                        <input type="checkbox" name="is_fragile" value="1" {{ old('is_fragile', $product->is_fragile) ? 'checked' : '' }} class="rounded border-gray-300 text-brand-red focus:ring-brand-red"> 
                            ...
                            <div class="space-y-2">
                                ...
                                <label class="flex items-center gap-3 cursor-pointer py-1">
                                    <input type="checkbox" name="payment_method_ids[]" value="{{ $pm->id }}"
                                           {{ in_array($pm->id, $productPaymentIds) ? 'checked' : '' }}
                        ...
                        <div x-show="tab === 'advanced'" style="display: none;" x-transition.opacity>
                            <div class="space-y-4 max-w-lg">
                                <div>
                                    ...
                                    <textarea name="purchase_note" rows="2" class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:border-brand-red outline-none">{{ old('purchase_note', $product->purchase_note) }}</textarea>
                                </div>
                                <div class="flex items-center gap-4">
                                    <label class="w-1/3 text-sm text-gray-600 font-medium">Menu order</label>
                                    <input type="number" name="sort_order" value="{{ old('sort_order', $product->sort_order) }}" class="w-2/3 border border-gray-200 rounded-md px-3 py-1.5 focus:border-brand-red outline-none">
                                </div>
                                <div class="pt-2">
                                    <label class="flex items-center gap-2 cursor-pointer text-gray-700 text-sm font-medium">
                                        <input type="checkbox" name="enable_reviews" value="1" {{ old('enable_reviews', $product->enable_reviews) ? 'checked' : '' }} class="rounded border-gray-300 text-brand-red focus:ring-brand-red h-4 w-4">
                                        Enable reviews
                                    </label>
                                </div>
                            </div>
        ...
        <div class="w-full lg:w-80 shrink-0 space-y-6">
                ...
                <div class="space-y-3 text-sm text-gray-700">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <select name="visibility" class="border border-gray-200 rounded px-2 py-1 outline-none focus:border-brand-red font-medium">
                            <option value="shop_search" {{ $product->visibility === 'shop_search' ? 'selected' : '' }}>Catalog & Search</option>
                            <option value="shop_only" {{ $product->visibility === 'shop_only' ? 'selected' : '' }}>Catalog only</option>
                            <option value="search_only" {{ $product->visibility === 'search_only' ? 'selected' : '' }}>Search only</option>
                    ...
                    <div class="flex items-center justify-between pt-2">
                        <label class="cursor-pointer flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="text-brand-red border-gray-300 rounded focus:ring-brand-red">
                            Active
                        </label>
                        <button type="submit"
                ...
                <div class="max-h-48 overflow-y-auto space-y-1 pr-2 custom-scrollbar text-sm">
                    <template x-for="cat in categories" :key="cat.id">
                        <label class="flex items-center gap-2 cursor-pointer p-1 hover:bg-gray-50 rounded">
                            <input type="radio" :id="'cat_radio_' + cat.id" name="category_id" :value="cat.id" :checked="cat.id == {{ old('category_id', $product->category_id ?? 'null') }}" class="text-brand-red bg-white border-gray-300 focus:ring-brand-red">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
                ...
                <div class="flex gap-2 mb-1">
                    <input type="text" name="material" value="{{ old('material', $product->material) }}" placeholder="e.g. Acrylic, PVC, Canvas" class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm focus:border-brand-red outline-none">
                </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('productForm', () => ({
            title: @json(old('name', $product->name)),
            slug: @json(old('slug', $product->slug)),
            shortDesc: @json(old('short_description', $product->short_description ?? '')),
            ...
            isVirtual: @json((bool) old('is_virtual', $product->is_virtual)),
            isDownloadable: @json((bool) old('is_downloadable', $product->is_downloadable)),
            tab: 'general',
            
            tierPricingEnabled: @json($product->priceTiers->whereNull('product_variation_id')->count() > 0),
            priceTiers: @json($product->priceTiers->whereNull('product_variation_id')->values()->toArray()),

            // Page Display Configuration (defaults built in controller; avoids Blade json-directive parse bugs)
            pageSettings: @json($pageSettingsForJs),
            ...
            // Tags
            newTag: '',
            selectedTags: @json($product->tags),
