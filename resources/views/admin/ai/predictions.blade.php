...
@section('content')
    <div class="flex items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.ai.overview') }}?period={{ $period ?? 'last_30_days' }}"
    ...
    @else
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow group">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-10 h-10 rounded-xl bg-gray-50 group-hover:bg-red-50 flex items-center justify-center transition-colors">
                            <i class="bi {{ $icon }} text-brand-red text-lg"></i>
                        </div>
                ...
                @else
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-700">{{ $salesForecast['model'] ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Forecast days</span>
                            <span class="font-medium text-gray-700">{{ $salesForecast['forecast_days'] ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Total predicted</span>
                            <span class="font-medium text-gray-900">Rs. {{ number_format($salesForecast['total_predicted_revenue'] ?? 0, 0) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Avg / day</span>
                            <span class="font-medium text-gray-700">Rs. {{ number_format($salesForecast['avg_daily_revenue'] ?? 0, 0) }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">History used</span>
                            <span class="font-medium text-gray-700">{{ $salesForecast['data_points_used'] ?? 'N/A' }} days</span>
                        </div>
                    </div>
                @endif
            </div>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @foreach($topProducts as $row)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-3 py-2 text-gray-700 font-medium">{{ $row['rank'] ?? '' }}</td>
                                        <td class="px-3 py-2 text-gray-900 font-medium">{{ $row['name'] ?? '' }}</td>
                                        <td class="px-3 py-2 text-right text-gray-700">{{ $row['quantity_sold'] ?? 0 }}</td>
                                        <td class="px-3 py-2 text-right text-gray-900 font-medium">Rs. {{ number_format($row['revenue'] ?? 0, 0) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                                ...
                                @foreach($stockAlerts as $row)
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-2 text-right text-gray-700">
                                            {{ array_key_exists('current_stock', $row) && $row['current_stock'] !== null ? number_format((float) $row['current_stock'], 0) : 'N/A' }}
                                        </td>
                                        <td class="px-3 py-2 text-right text-gray-900 font-medium">
                                            {{ array_key_exists('days_until_stockout', $row) && $row['days_until_stockout'] !== null ? number_format((float) $row['days_until_stockout'], 0) : 'N/A' }}
                                        </td>
                                        <td class="px-3 py-2">
                                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium {{ $class }}">{{ ucfirst($urgency) }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
            ...
            {{-- Category trends + Repeat customers --}}
            <div class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div class="flex items-center gap-2 mb-3">
                    ...
                    @else
                        <div class="space-y-2 text-sm">
                            @foreach(array_slice($categoryTrends, 0, 8) as $row)
                                @php
                                ...
                                <div class="flex items-center justify-between gap-3 bg-gray-50 rounded-lg px-3 py-2">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500">Qty: {{ $row['quantity'] ?? 0 }} | Revenue: Rs. {{ number_format($row['revenue'] ?? 0, 0) }}</p>
                                    </div>
                                    <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium {{ $class }}">
                                        {{ number_format($row['change_percent'] ?? 0, 1) }}%
                                    </span>
                                </div>
                            @endforeach
                        </div>
                    ...
                    @else
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div class="bg-gray-50 rounded-lg p-3">
                                ...
                                <p class="text-gray-900 font-bold">{{ $customerInsights['champions'] ?? 0 }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3">
                                <p class="text-gray-500 text-xs">Loyal</p>
                                <p class="text-gray-900 font-bold">{{ $customerInsights['loyal'] ?? 0 }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3">
                                <p class="text-gray-500 text-xs">At risk</p>
                                <p class="text-gray-900 font-bold">{{ $customerInsights['at_risk'] ?? 0 }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-lg p-3">
                                <p class="text-gray-500 text-xs">Likely repeat</p>
                                <p class="text-gray-900 font-bold">{{ $customerInsights['likely_repeat'] ?? 0 }}</p>
                            </div>
                        </div>
                        @if(!empty($customerInsights['total_customers']))
                            <p class="text-xs text-gray-500 mt-3">Total customers scored: {{ $customerInsights['total_customers'] }}</p>
                        @endif
                    @endif
                </div>
            </div>
        ...
        {{-- Recommendations --}}
        @php $recs = $overview['recommendations'] ?? []; @endphp
        @if(!empty($recs))
