    ...
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-gray-100">
        ...
        <tr class="hover:bg-gray-50/60 transition">
          <td class="px-5 py-3 text-gray-400 font-mono text-xs">{{ $method->sort_order }}</td>
          <td class="px-5 py-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                @endif
                <p class="text-xs text-gray-400 font-mono mt-0.5">{{ $method->code }}</p>
              </div>
            </div>
          </td>
          <td class="px-5 py-3">
            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $method->type === 'online' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600' }}">
              {{ ucfirst($method->type) }}
            </span>
          </td>
          <td class="px-5 py-3">
          ...
          <td class="px-5 py-3">
            <div class="flex items-center justify-end gap-2">
              {{-- Toggle --}}
              <form action="{{ route('admin.payments.toggle', $method) }}" method="POST">
                @csrf
                <button type="submit"
                        class="inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-xs font-semibold transition
                          {{ $method->is_active ? 'border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100' : 'border-green-200 bg-green-50 text-green-700 hover:bg-green-100' }}">
              ...
              @if(! in_array($method->code, ['cod', 'payhere']))
  ...
  <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
      ...
      <span class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold
        {{ $settings->mode === 'live' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">
        <span class="h-1.5 w-1.5 rounded-full {{ $settings->mode === 'live' ? 'bg-green-500' : 'bg-amber-500' }}"></span>
        {{ strtoupper($settings->mode) }} Mode
      </span>
    </div>

    <form action="{{ route('admin.payments.payhere.update') }}" method="POST" class="p-6 space-y-6">
      @csrf
      ...
      <div>
        ...
        <div class="flex gap-4">
          <label class="flex items-center gap-3 cursor-pointer rounded-xl border px-5 py-3 transition
            {{ $settings->mode === 'sandbox' ? 'border-amber-400 bg-amber-50' : 'border-gray-200 bg-white hover:bg-gray-50' }}">
            <input type="radio" name="mode" value="sandbox" {{ $settings->mode === 'sandbox' ? 'checked' : '' }}
          ...
          <label class="flex items-center gap-3 cursor-pointer rounded-xl border px-5 py-3 transition
            {{ $settings->mode === 'live' ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-white hover:bg-gray-50' }}">
            <input type="radio" name="mode" value="live" {{ $settings->mode === 'live' ? 'checked' : '' }}
                   class="text-brand-red focus:ring-brand-red h-4 w-4">
            <div>
      ...
      <div class="grid sm:grid-cols-2 gap-8">
        ...
        <div class="space-y-4 p-4 rounded-xl border border-amber-200 bg-amber-50/50">
          ...
          <div>
            ...
            <input type="text" name="merchant_id_sandbox" value="{{ old('merchant_id_sandbox', $settings->merchant_id_sandbox) }}"
          ...
          <div>
            ...
            <input type="password" name="merchant_secret_sandbox"
                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-brand-red focus:ring-2 focus:ring-brand-red/20"
                   placeholder="{{ $settings->merchant_secret_sandbox ? '••••••••• (saved — leave blank to keep)' : 'Enter secret' }}">
            @if($settings->merchant_secret_sandbox)
        ...
        <div class="space-y-4 p-4 rounded-xl border border-green-200 bg-green-50/50">
          ...
          <div>
            ...
            <input type="text" name="merchant_id_live" value="{{ old('merchant_id_live', $settings->merchant_id_live) }}"
                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-brand-red focus:ring-2 focus:ring-brand-red/20"
          ...
          <div>
            ...
            <input type="password" name="merchant_secret_live"
                   class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-900 outline-none focus:border-brand-red focus:ring-2 focus:ring-brand-red/20"
                   placeholder="{{ $settings->merchant_secret_live ? '••••••••• (saved — leave blank to keep)' : 'Enter secret' }}">
            @if($settings->merchant_secret_live)
      ...
      <div class="rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3">
        ...
        <p class="text-sm font-mono text-blue-900 select-all">{{ config('app.url') }}/api/payments/payhere/notify</p>
      </div>
